...
make_target = make_pipeline(p.ColumnSelector('imbalancePriceAmountGBP'),
                            p.OffsetGenerator('horizion', HORIZIONS),
                            p.AlignPandas(max(LAGS), max(HORIZIONS)),
                            p.AsMatrix(),
                            StandardScaler())
...
make_features = make_pipeline(p.ColumnSelector(['imbalancePriceAmountGBP',
